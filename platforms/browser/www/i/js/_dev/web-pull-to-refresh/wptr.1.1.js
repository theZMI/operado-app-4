var WebPullToRefresh = null
$(function () {
    WebPullToRefresh = (function () {
        'use strict';
        ...
        var _panStart = function (e) {
            if (
                $(window).scrollTop()
                ||
                $('#main-menu').hasClass('menu-is-open')
            ) {
                return;
            }
        ...
        var _convertToPercent = function (distance) {
            var h = $(document).height();
            var n = 1 / (h / 3000);
            var ret = parseInt(n * +distance);
        ...
        var _panDown = function (e) {
            ...
            var percent = _convertToPercent(pan.distance);
            if (percent >= 100) {
                e.preventDefault();
                ...
                $('.ptr-loading-pulling-percent').html(percent + '%');
        ...
        var _setContentPan = function () {
            ...
            var offset = (pan.distance - options.ptrEl.offsetHeight);
            offset = Math.min(offset, 35);
            options.ptrEl.style.transform = options.ptrEl.style.webkitTransform = 'translate3d( 0, ' + offset + 'px, 0 )';
        };
