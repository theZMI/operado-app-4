class ClientRegistrationForm {
    ...
    private initFormStep1(): void {
        let form = $('#i-client-registration-form_step-1');
        let self = this;
    ...
    private initFormStep2(): void {
        let self = this;
        let form = $('#i-client-registration-form_step-2');
        ...
        pinForm.init(
            ...
            function (isCorrectPin) {
                ...
                if (isCorrectPin) {
                    ...
                    self.API.get(
                        'clients/is_phone_busy/' + encodeURIComponent(PhoneFilter(phone as string)),
                        [],
                        function (isBusy) {
                            let uri = +isBusy ? 'clients/regenerate_password' : 'clients';
                            self.API.post(
                                uri,
                                {
                                    phone: phone,
